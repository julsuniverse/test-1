...
<div class="container">
    <div class="row info">
        ...
        <h1 class="titleOne"><?=$service->name;?></h1>
        <div class="infoPhotoTop"><img src="<?= $imgPath;?><?=$service->logo;?>" alt="<?=$service->name;?>"/></div>
        <?php Pjax::begin(['id' => 'my-pjax-table', 'timeout' => 500000]);?>
            <table id="pjax-table" class="table table-striped raiting one_comp_rait strip_reverse"  data-gisp="service">
                <tbody>
                    <tr>
                        <th>Сайт</th>
                        <td><?php if($service->site_link==1){?><a  target="_blank" href="<?=$service->site;?>"><?=$service->site;?></a><?php } else echo $service->site;?></td>
                    </tr>
                    <tr>
                        <th>Телефон</th>
                        <td><?=$service->tel;?></td>
                    ...
                    <tr>
                        <th>Рейтинг</th>
                        <td id="tablerait" data-id="<?=$service->id;?>"><?=$service->raiting;?></td>
                    </tr>
                    <tr>
                        <th>Отзывы</th>
                        <td id="tablerev"><?=$service->reviews;?></td>
                    </tr>
                    <tr>
                        <th>Группа VK</th>
                        <td><a  target="_blank" href="<?=$service->vk_group;?>"><?=$service->vk_group;?></a></td>
                    </tr>
                    <tr>
                        <th>Группа FB</th>
                        <td><a  target="_blank" href="<?=$service->fb_group;?>"><?=$service->fb_group;?></a></td>
                    </tr>
                    <tr>
                        <th>Люди</th>
                        <td><?=$service->peoples_string;?></td>
                    </tr>
                </tbody>
            </table>
           <?php Pjax::end();?> 
        </div>
        <div class="col-md-6 col-sm-12 infoPhoto"><img src="<?= $imgPath;?><?=$service->logo;?>" alt="<?=$service->name;?>"/></div>
        
    </div>
    <!--<div class="row"><div class="col-md-6 col-sm-12"><div class="strip"></div></div></div>-->
    <div class="row infoText">
        <div class="col-md-12 col-sm-12">
        <h2 class="infoTitle">О сервисе</h2>
            <?=$service->about;?>	
        </div>
    </div>
...
<div class="container container-reviews">
    ...
    <div  style="margin:0;" class="row" id="sortreviews">
        <div class="col-xs-12 col-sm-12 col-md-3"><h2 class="infoTitle">Отзывы о <?=$service->name;?></h2></div>
        ...
        <div  class="col-xs-12 col-sm-12 col-md-9 sort">
            <div class="row">
                ...
                <div class="col-lg-3 col-sm-3"><a href="<?=Url::toRoute(['service/service','alias'=>$alias, 'sort'=>'popular', 'sort_desc'=>'desc']);?>" <?php if($sort=="popular") echo 'class="active"';?>>Популярность</a></div>
                <?php } else {?>
                <div class="col-lg-3 col-sm-3"><a href="<?=Url::toRoute(['service/service','alias'=>$alias, 'sort'=>'popular']);?>" <?php if($sort=="popular") echo 'class="active"';?>>Популярность<?php if($sort_desc && $sort=="popular"){?><span class="glyphicon glyphicon-sort" aria-hidden="true"></span><?php }?></div>
                <?php }?>
                <?php if(!$sort && !$sort_desc){?>
                <div class="col-lg-2 col-sm-1" style="text-align: center;"><a href="<?=Url::toRoute(['service/service','alias'=>$alias, 'sort_desc'=>'desc']);?>" <?php if(!$sort) echo 'class="active"';?>>Дата</a></div>
                <?php } else {?>
                <div class="col-lg-2 col-sm-1" style="text-align: center;"><a href="<?=Url::toRoute(['service/service','alias'=>$alias]);?>" <?php if(!$sort) echo 'class="active"';?>>Дата<?php if($sort_desc && !$sort){?><span class="glyphicon glyphicon-sort" aria-hidden="true"></span><?php }?></a></div>
                <?php }?>
                <?php if($sort=='bad-good' && !$sort_desc){?>
                <div class="col-lg-3 col-sm-4"><a href="<?=Url::toRoute(['service/service','alias'=>$alias, 'sort'=>'bad-good', 'sort_desc'=>'desc']);?>" <?php if($sort=="bad-good") echo 'class="active"';?>>Негатив/Позитив</a></div>    
                <?php } else {?>
                <div class="col-lg-3 col-sm-4"><a href="<?=Url::toRoute(['service/service','alias'=>$alias, 'sort'=>'bad-good']);?>" <?php if($sort=="bad-good") echo 'class="active"';?>>Негатив/Позитив<?php if($sort_desc && $sort=="bad-good"){?><span class="glyphicon glyphicon-sort" aria-hidden="true"></span><?php }?></a></div>    
                <?php }?>
            </div>
            ...
            <div class="dropsort"><span>Сортировать по:</span>
                <div class="dropdown">
                  ...
                  <ul class="dropdown-menu" aria-labelledby="dropdownMenu1">
                    <li><a href="<?=Url::toRoute(['service/service','alias'=>$alias, 'sort'=>'popular']);?>" <?php if($sort=="popular") echo 'class="active"';?>>Популярность</a></li>
                    <li><a href="<?=Url::toRoute(['service/service','alias'=>$alias]);?>" <?php if(!$sort) echo 'class="active"';?>>Дата</a></li>
                    <li><a href="<?=Url::toRoute(['service/service','alias'=>$alias, 'sort'=>'bad-good']);?>" <?php if($sort=="bad-good") echo 'class="active"';?>>Негатив/Позитив</a></li>
                  </ul>
                </div>
            </div>
        </div>
    ...
    <div class="add_comment" id="add-review">
        ...
        <?php $f=ActiveForm::begin(['options'=>['data-pjax'=>true]]);?>
            <div class="row">
                <div class="col-md-7">
                    <?=$f->field($model, 'text')->textArea(['placeholder'=>'Напишите здесь Ваше мнение о сервисе...', 'class'=>'review_text'])->label('');?>
                </div>
                <div class="col-md-5">
...
<?php if($wall || $fb_wall){
    if ($this->beginCache($service->id."servicewall", ['duration' => $wall_cach])) {?>
<?php 
